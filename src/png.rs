...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk, Error> {
            ...
            .position(|chunk| chunk.chunk_type().to_string() == chunk_type)
        {
            ...
            Err(Error::NoChunkOfGivenTypeFound)
        }
    }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let chunks_bytes_len = self
            .chunks
            .iter()
            .fold(0, |acc, chunk| acc + chunk.length() + 12);
        let chunks_as_bytes = self.chunks.iter().map(|chunk| chunk.as_bytes()).fold(
            Vec::with_capacity(chunks_bytes_len as usize),
            |mut acc, v| {
                acc.extend(v);
                acc
            },
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        ...
        while chunk_start < bytes.len() {
            let chunk = Chunk::try_from(&bytes[chunk_start..])?;
            chunk_start += chunk.length() as usize + 12;
            chunks.push(chunk);
        }
...
impl Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{} chunk(s)", self.chunks.len())
    }
}
